...
while true
	...
	if File::directory?(name)
		...
		if BinaryConstants::DuelModules.index(asm["mod"]) == nil
			next
		end
		...
		while true
			...
			rsite = droid_select(sample["registers"], lambda{|e| e["name"] == "r8"})[0]
			register = sample["registers"][rsite]
			if register["number"] == 0xffffffff
				lasm = poly_clone(asm) # lasm | lead-asm
				lasm["ocurance-site"] = ssite
				#lasm["register-mention"] = register["name"]
				rsite = droid_select(sample["registers"], lambda{|e| e["name"] == "rbp"})[0]
				register = sample["registers"][rsite]
				lasm["call-site"] = register["number"]
				asms += [lasm]
				asm_bss += [lasm["bs"].to_s(16)]
				if asm["mod"] == "syscall"
					asm_bss += [lasm["completion"].to_s(16)]
				else
					asm_bss += [lasm["destination"].to_s(16)]
				end
			end
			ssite += 1
		end
