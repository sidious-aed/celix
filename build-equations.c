...
quadrant main(quadrant naof_params, source_vecter params) {
	...
	node_meta(&grid, "assemblies/equations", &nm);
	printf("nm | %lu\n", &nm);
	sec nonce_name[1000];
	...
	while(true) {
		...
		if(compair_spaces(extension, naof_extension_secs, "asm", 3)) {
			...
			comand_site = 0;
			add_to_entree("./sequences assemblies/equations/", 33, comand, &comand_site);
			...
			add_to_entree(" 0 > /dev/null", 14, comand, &comand_site);
			syscall(unix_write, 1, comand, (comand_site - 12));
			syscall(unix_write, 1, "\n", 1);
			system(comand);
